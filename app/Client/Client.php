        ...
        connect($wsProtocol."://{$this->configuration->host()}:{$this->configuration->port()}/cloakr/control?authToken={$authToken}&version={$cloakrVersion}", [], [
            ...
            ->then(function (WebSocket $clientConnection) use ($sharedUrl, $subdomain, $serverHost, $deferred, $authToken) {
                ...
                $connection->on('authenticated', function ($data) use ($deferred, $sharedUrl) {
                    ...
                    $this->logger->info("Dashboard-URL:\t\t<options=bold>http://127.0.0.1:".config()->get('cloakr.dashboard_port')."</>");
                    $this->logger->info("Cloakr-URL:\t\t<options=bold>http://{$data->subdomain}.{$host}{$httpPort}</>");
                    $this->logger->info("Cloakr-URL:\t\t<options=bold>https://{$data->subdomain}.{$host}</>");
                    $this->logger->line('');
        ...
        connect($wsProtocol."://{$this->configuration->host()}:{$this->configuration->port()}/cloakr/control?authToken={$authToken}&version={$cloakrVersion}", [], [
            ...
            ->then(function (WebSocket $clientConnection) use ($port, $deferred, $authToken) {
                ...
                $connection->on('authenticated', function ($data) use ($deferred, $port) {
                    ...
                    $this->logger->info("Cloakr-URL:\t\t<options=bold>tcp://{$host}:{$data->shared_port}</>");
                    $this->logger->line('');
