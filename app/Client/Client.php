...
class Client
{
    ...
    public function share(string $sharedUrl, array $subdomains = [])
    {
        ...
        foreach ($subdomains as $subdomain) {
            $this->connectToServer($sharedUrl, $subdomain, config('cloakr.auth_token'));
        }
    }

    public function sharePort(int $port)
    {
        $this->connectToServerAndShareTcp($port, config('cloakr.auth_token'));
    }
        ...
        connect($wsProtocol."://{$this->configuration->host()}:{$this->configuration->port()}/cloakr/control?authToken={$authToken}", [], [
            ...
            ->then(function (WebSocket $clientConnection) use ($sharedUrl, $subdomain, $deferred, $authToken) {
                ...
                $connection->on('authenticated', function ($data) use ($deferred, $sharedUrl) {
                    ...
                    $this->logger->info("Dashboard-URL:\t\thttp://127.0.0.1:".config()->get('cloakr.dashboard_port'));
                    $this->logger->info("Cloakr-URL:\t\t{$httpProtocol}://{$data->subdomain}.{$host}");
                    $this->logger->line('');
        ...
        connect($wsProtocol."://{$this->configuration->host()}:{$this->configuration->port()}/cloakr/control?authToken={$authToken}", [], [
            ...
            ->then(function (WebSocket $clientConnection) use ($port, $deferred, $authToken) {
                ...
                $connection->on('authenticated', function ($data) use ($deferred, $port) {
                    ...
                    $this->logger->info("Cloakr-URL:\t\ttcp://{$host}:{$data->shared_port}.");
                    $this->logger->line('');
