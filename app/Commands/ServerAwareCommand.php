...
abstract class ServerAwareCommand extends Command
{
    ...
    {
        return $this->option('server-host') ?? config('cloakr.servers.'.$this->option('server').'.host', 'localhost');
    }

    protected function getServerPort()
    {
        return $this->option('server-port') ?? config('cloakr.servers.'.$this->option('server').'.port', 8080);
    }
}
