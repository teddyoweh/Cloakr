...
class LoggedRequest implements \JsonSerializable
{
    ...
    {
        if (empty(config()->get('cloakr.skip_body_log.status'))) {
            return false;
        }

        return Str::is(config()->get('cloakr.skip_body_log.status'), $this->parsedResponse->getStatusCode());
    }

    protected function skipByContentType(): bool
    {
        if (empty(config()->get('cloakr.skip_body_log.content_type'))) {
            return false;
        }
        ...
        return Str::is(config()->get('cloakr.skip_body_log.content_type'), $contentType);
    }

    protected function skipByExtension(): bool
    {
        if (empty(config()->get('cloakr.skip_body_log.extension'))) {
            return false;
        }

        return Str::is(config()->get('cloakr.skip_body_log.extension'), $this->parsedRequest->getUri()->getPath());
    }

    protected function skipBySize(): bool
    {
        $configSize = $this->getConfigSize(config()->get('cloakr.skip_body_log.size', '1MB'));
