...
class Factory
{
    ...
    {
        $adminCondition = 'request.headers.get("Host") matches "/^'.config('cloakr.admin.subdomain').'\\\\./i"';
    ...
    protected function bindSubdomainGenerator()
    {
        app()->singleton(SubdomainGenerator::class, function ($app) {
            return $app->make(config('cloakr.admin.subdomain_generator'));
        });
    ...
    protected function bindUserRepository()
    {
        app()->singleton(UserRepository::class, function () {
            return app(config('cloakr.admin.user_repository'));
        });
    ...
    protected function bindSubdomainRepository()
    {
        app()->singleton(SubdomainRepository::class, function () {
            return app(config('cloakr.admin.subdomain_repository', DatabaseSubdomainRepository::class));
        });
    ...
    protected function bindLoggerRepository()
    {
        app()->singleton(LoggerRepository::class, function () {
            return app(config('cloakr.admin.logger_repository', NullLogger::class));
        });
    ...
    protected function bindDomainRepository()
    {
        app()->singleton(DomainRepository::class, function () {
            return app(config('cloakr.admin.domain_repository', DatabaseDomainRepository::class));
        });
    ...
    protected function bindDatabase()
    {
        app()->singleton(DatabaseInterface::class, function () {
            ...
            return $factory->openLazy(
                config('cloakr.admin.database', ':memory:'),
                null,
                $options,
            );
        });
    ...
    {
        config()->set('cloakr.admin.validate_auth_tokens', $validate);
    ...
    protected function registerStatisticsCollector()
    {
        ...
        app()->singleton(StatisticsRepository::class, function () {
            return app(config('cloakr.admin.statistics.repository', DatabaseStatisticsRepository::class));
        });
        ...
        $intervalInSeconds = config('cloakr.admin.statistics.interval_in_seconds', 3600);
